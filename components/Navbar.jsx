    ...
    return (
        <header className={`fixed inset-x-0 z-50 transition-all duration-500 ease-in-out ${scrolled ? 'top-2 sm:top-4' : 'top-0'}`}>
            <div className="max-w-6xl mx-auto px-4 sm:px-6">
                <nav className={`flex items-center justify-between transition-all duration-500 ease-in-out ${scrolled ? 'bg-white/80 backdrop-blur-xl shadow-[0_8px_30px_rgb(0,0,0,0.04)] rounded-full px-5 py-3 md:px-8 border border-white/50' : 'bg-transparent py-5 px-2 md:px-4'}`}>
                    
                    {/* Logo */}
                    <Link href="/" className="flex items-center gap-2.5 group relative z-50">
                        <div className="w-10 h-10 rounded-xl bg-gradient-to-br from-emerald-500 to-teal-600 flex items-center justify-center text-white shadow-lg shadow-emerald-500/30 group-hover:rotate-12 group-hover:scale-110 transition-all duration-300">
                            <i className="fas fa-leaf text-lg"></i>
                        </div>
                        <span className="font-bold text-xl tracking-tight text-slate-900">
                            Smart<span className="text-emerald-600">Desa</span>
                        </span>
                    </Link>
