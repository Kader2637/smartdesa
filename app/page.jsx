...
export default function Home() {
  const { surats, products, addToCart, isInitialized } = useSmartDesa();
  ...
  const handleLacakSurat = () => {
    ...
    setTimeout(() => {
      ...
      const inputId = normalizedID.startsWith('#') ? normalizedID : `#${normalizedID}`;

      const surat = surats.find(s => s.id === inputId);
      ...
      if (surat) {
        if (surat.status === 'selesai') {
          setTrackingResult({
            ...
            desc: `Surat ${surat.type} Anda telah ditandatangani secara elektronik (TTE) dan siap diunduh di dashboard warga.`,
            step: 4
          });
        } else if (surat.status === 'menunggu_kades') {
          setTrackingResult({
            id: surat.id,
            ...
            desc: `Surat ${surat.type} Anda telah diverifikasi RT dan sedang menunggu TTE dari Kepala Desa.`,
            step: 2
          });
        } else if (surat.status === 'menunggu_rt') {
           setTrackingResult({
            id: surat.id,
            status: 'Menunggu RT',
            icon: 'fa-clock',
            ...
            desc: `Surat ${surat.type} Anda masuk ke sistem. Sedang menunggu persetujuan Ketua RT setempat.`,
            step: 1
          });
          ...
          setTrackingResult({
            ...
            desc: `Maaf, Surat ${surat.type} Anda ditolak karena berkas tidak lengkap atau alasan lainnya.`,
            step: 0
          });
        }
      } else {
  ...
  const umkmData = isInitialized && products.length > 0 ? products.slice(0, 4) : [];
            ...
            {umkmData.map((item, idx) => (
              ...
              >
                ...
                <div className="px-2">
                  ...
                  <p className="text-lg font-extrabold text-emerald-600">Rp {item.harga.toLocaleString('id-ID')}</p>
                </div>
              </motion.div>
            ))}
          </div>
        ...
        >
          ...
          >
            ...
            <div className="p-8">
              ...
              <p className="text-slate-500 text-sm mb-6 pb-6 border-b border-slate-100">Diproduksi dengan bahan lokal berkualitas unggul murni dari desa oleh UMKM {modalData.penjual}. Stok: {modalData.stock}</p>

              <div className="flex justify-between items-center bg-slate-50 p-4 rounded-2xl border border-slate-100">
                <div>
                  <p className="text-[10px] uppercase font-bold text-slate-400 mb-0.5">Harga Produk</p>
                  <p className="text-2xl font-extrabold text-emerald-600">Rp {modalData.harga.toLocaleString('id-ID')}</p>
                </div>
                <button 
                  onClick={() => {
                    addToCart(modalData);
                    alert(`${modalData.nama} ditambahkan ke keranjang.`);
                  }}
                  className="w-12 h-12 bg-slate-900 hover:bg-emerald-600 text-white rounded-xl flex items-center justify-center transition-colors shadow-lg"
                >
